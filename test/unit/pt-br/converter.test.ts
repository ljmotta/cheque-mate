import { getAuxiliarWord } from '../../../src/locale/pt-br/converter'
import { expect } from '../../utils/common'

describe('getAuxiliarWord', () => {
	const shouldTests = [
	...
	shouldTests.forEach(test => {
		it('should get auxiliary word', () => {
			const data = getAuxiliarWord(test.args.digitPosition, test.args.word)
	...
	it('shouldn\'t get auxiliary word due to missing \'milhao\'', () => {
		expect(() => getAuxiliarWord(7, '')).to.throw('Missing auxiliary word for input. Auxiliary words: mil')
	})
})
